...
void setup() {
    ...
    // clock one before, minute exact. flash 13:38:40 for 13:39:00
    // rtc.adjust(DateTime(F("Jun 13 2018"), F("13:57:00")));
    tick();

    /***/
    //total_accuracy_secs = time.unixtime();
    //EEPROM.put(0x8, total_accuracy_secs);
    /***/

    EEPROM.get(0x8, total_accuracy_secs);
...
void loop() {
    ...
    if(unix_time != total_accuracy_secs) {
        if((time - DateTime(total_accuracy_secs)).days() == 30) {
